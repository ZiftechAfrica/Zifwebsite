...
if (!defined('PAYSTACK_SECRET_KEY')) {
    $paystackKey = '';
    $result = $conn->query("SELECT secret_key FROM payment_settings WHERE gateway = 'paystack' LIMIT 1");
    if ($result && $row = $result->fetch_assoc()) {
        $paystackKey = $row['secret_key'] ?? '';
    }
    if ($paystackKey === '') {
        $paystackKey = 'sk_test_xxxxxxxxxxxxxxxxxxxxxxxx';
    }
    define('PAYSTACK_SECRET_KEY', $paystackKey);
}
